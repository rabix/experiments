...
def test_cmd_line():
    ...
    #job containing tool
    CMD1 = ['run', '--job', '/home/sinisa/devel/CWL/executors/experiments/tests/test-data/test-cmdline/bwa-mem.yml'] # abspath --job, no verbose
    tool1 = get_tool(CMD1)
    CMD2 = ['run', '-v', '--job=/home/sinisa/devel/CWL/executors/experiments/tests/test-data/test-cmdline/bwa-mem.yml'] # abspath --job=, verbose
    tool2 = get_tool(CMD2)
...
@nottest
def test_from_url():
    a = from_url('https://s3.amazonaws.com/boysha/pipeline_test_bwa_freebayes.json')
    b = from_url('/home/sinisa/devel/CWL/executors/experiments/examples/bwa-mem.yml')
    c = from_url('../../examples/bwa-mem.yml')